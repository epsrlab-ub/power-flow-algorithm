# -*- coding: utf-8 -*-
"""
Created on Thu Oct 24 06:48:44 2024
...
def dpf_method(load_data, line_data, slack_bus_voltage=1, tolerance=1e-6, max_iter = 100, runs = 10):
    ...
    # Begin Decoupled Power Flow method for multiple runs
    for run in range(runs):
        ...
        for iteration in range(max_iter):
            ...
            # Fast Decoupled Algorithm: make J2 and J3 zeros
            J1 = Jacobian_inv[:num_buses-1, :num_buses-1]
            J2 = np.zeros((num_buses-1, num_buses-1))
            J3 = np.zeros((num_buses-1, num_buses-1))
            J4 = Jacobian_inv[num_buses-1:, num_buses-1:]

            delta_mismatch = np.dot(J1, P_mismatch[1:]) + np.dot(J2, Q_mismatch[1:])
            V_mismatch = np.dot(J3, P_mismatch[1:]) + np.dot(J4, Q_mismatch[1:])

            V_new = np.abs(v[1:]) + V_mismatch
            delta_new = np.angle(v[1:]) + delta_mismatch

            v[1:] = V_new * np.exp(1j * delta_new)  # Polar to rectangular conversion

            # End timing the iteration
            iter_time = time.time() - iter_start_time

            # Check for max_difference
            max_diff = np.max(np.abs(V_mismatch))
            ...
            # Print max voltage difference and iteration time
            print(f"Iteration {iteration + 1}: max voltage difference = {max_diff:.10f}")
            print(f"Time taken till iteration {iteration + 1}: {iter_time:.4f} seconds")

            if max_diff <= tolerance:  # Check if convergence is achieved
