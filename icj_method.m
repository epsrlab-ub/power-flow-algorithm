% Define the function for the Inexact and Constant Jacobian Method for distribution network analysis.
function [v, iteration] = icj_method(load_data, line_data, slack_bus_voltage, tolerance, max_iter, runs, v_base, s_base)
    ...
    for run = 1:runs
        ...
        Jacobian = build_inexact_Jacobian(Y_bus, num_buses);
        
        Jacobian_time(run) = toc(Jacobian_start_time);

        % Measure the computational time
        start_time = tic;
       
        % Start NR Iterations
        for iteration = 1:max_iter % Iterate up to max_iter times to perform the ICJ algorithm.
            ...
            % cumulative_iter_times = cumsum(iter_time(1:iteration));
           
            % Get the cumulative sum of each iteration for plotting, same
            % as above
            %if iteration == 1
            %    cumulative_iter_times(iteration) = iter_time(run);
            %else
            %    cumulative_iter_times(iteration) = cumulative_iter_times(iteration - 1) + iter_time(run);
            %end
    ...
    fprintf('\nThe Jacobian was computed in %f secs\n', mean(Jacobian_time));
    ...
    % grid on;
    filename1 = sprintf('icj_vol_profile_%d_bus.fig', num_buses);
    filename2 = sprintf('icj_vol_profile_%d_bus.png', num_buses);
    savefig(fig, fullfile(pwd, 'Figures', filename1));
    exportgraphics(gcf, fullfile(pwd, 'Figures', filename2), 'Resolution', 300);
    close(fig);
    ...
    % grid on;
    filename1 = sprintf('icj_maxE_Comp_%d_bus.fig', num_buses);
    filename2 = sprintf('icj_maxE_Comp_%d_bus.png', num_buses);
    savefig(fig, fullfile(pwd, 'Figures', filename1));
    exportgraphics(gcf, fullfile(pwd, 'Figures', filename2), 'Resolution', 300);
    close(fig);
    
    % Write results to a text file with UTF-8 encoding
    fid_txt = fopen(sprintf('Result_files/icj_bus_voltage_%d_bus.txt', num_buses), 'w', 'n', 'UTF-8');
    ...
    writetable(T, sprintf('Result_files/icj_bus_voltage_%d_bus.csv', num_buses));
