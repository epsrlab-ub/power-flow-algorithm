# -*- coding: utf-8 -*-
"""
Created on Thu Oct 24 06:25:41 2024
...
def zbus_method(load_data, line_data, slack_bus_voltage=1, tolerance=1e-6, max_iter = 100, runs = 10):
    ...
    # Begin Z-bus Method for multiple runs
    for run in range(runs):
        # Initialize bus voltages
        v = np.ones(num_buses, dtype=complex)  # Initialize all voltages to 1.0 p.u.
        v[0] = slack_bus_voltage  # Set the slack bus voltage

        # Construct the Y_bus matrix using the helper function (to be defined)
        ...
        # Z-bus (Fixed-Point iteration) method
        for iteration in range(max_iter):
            # Start timing for the iteration
            iter_start_time = time.time()

            # Calculate current injection for all buses using I_i = sum(Y_ij * V_j)
            I = -(P_load - 1j * Q_load) / np.conj(v)

            # Separate currents into slack and non-slack parts
            I_m = I[1:]  # Non-slack currents
            V_old = v[1:].copy()  # Old voltages for non-slack buses

            v[1:] = np.linalg.solve(Y_mm, I_m - Y_ms * v[0])  # Solve for voltages

            # End timing the iteration
            iter_time = time.time() - iter_start_time

            # Check for convergence
            max_diff = np.max(np.abs(v[1:] - V_old))
