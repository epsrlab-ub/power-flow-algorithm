% Define the function for the zbus method for distribution network analysis.
function [v, iteration] = zbus_method(load_data, line_data, slack_bus_voltage, tolerance, max_iter, runs, v_base, s_base)
   ...
   % To handle only the iterations that were executed
    ...
    % grid on;
    filename1 = sprintf('zbus_vol_profile_%d_bus.fig', num_buses);
    filename2 = sprintf('zbus_vol_profile_%d_bus.png', num_buses);
    savefig(fig, fullfile(pwd, 'Figures', filename1));
    exportgraphics(gcf, fullfile(pwd, 'Figures', filename2), 'Resolution', 300);
    close(fig);
    ...
    % grid on;
    filename1 = sprintf('zbus_maxE_Comp_%d_bus.fig', num_buses);
    filename2 = sprintf('zbus_maxE_Comp_%d_bus.png', num_buses);
    savefig(fig, fullfile(pwd, 'Figures', filename1));
    exportgraphics(gcf, fullfile(pwd, 'Figures', filename2), 'Resolution', 300);
    close(fig);
    
    % Write results to a text file with UTF-8 encoding
    fid_txt = fopen(sprintf('Result_files/zbus_bus_voltage_%d_bus.txt', num_buses), 'w', 'n', 'UTF-8');
    ...
    writetable(T, sprintf('Result_files/zbus_bus_voltage_%d_bus.csv', num_buses));
